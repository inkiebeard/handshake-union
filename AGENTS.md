...
9. **Link preview anchors must always target the original URL.** `LinkPreview` must use the `url` prop (the message's `link_url`) as the anchor `href`. Never substitute a metadata-supplied field (`og:url`, `og:video`, `twitterUrl`, etc.) as the click destination — a malicious HTTPS page can set these to redirect users to an arbitrary host (phishing). OG/Twitter metadata is for display purposes only.
...
- **`CHANGELOG.md`** is the canonical record of every meaningful change to the project. **Add a changelog entry only when raising a PR or when explicitly asked to wrap up a feature or fix** — not per commit, not per prompt, and not for in-progress work. One dated entry per PR/feature, following the existing format: `### YYYY-MM-DD — <short title>`, with `- **Added/Fixed/Updated/Changed/Removed:**` bullets, an optional `- **Rationale:**` line where the reason isn't obvious, and a `- **Affected:**` line listing touched files.
