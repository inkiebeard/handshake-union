...
- **Added:** `og-preview` Supabase Edge Function — fetches the target URL server-side (10 s timeout, `AbortController`), extracts Open Graph meta tags (`og:title`, `og:description`, `og:image`), falls back to Twitter Card equivalents (`twitter:title`, etc.). Authenticated callers only: JWT role checked via the Supabase gateway. (Metadata fields and HTML byte limit subsequently expanded — see 2026-03-20 entry.)
